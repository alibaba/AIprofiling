...
bool ElfParser::parseFile(CodeCache* cc, const char* base, const char* file_name, bool use_debug) {
    ...
    if (addr == MAP_FAILED) {
        WARNING("Could not parse symbols from %s: %s", file_name, strerror(errno));
    } else {
...
bool ElfParser::loadSymbolsFromDebuginfodCache(const char* build_id, const int build_id_len) {
    ...
    if (debuginfod_cache_len + build_id_len + strlen("/debuginfo") >= sizeof(path)) {
        WARNING("Path too long, skipping loading symbols: %s", debuginfod_cache);
        return false;
    }
...
CodeCacheArray Symbols::_native_libs;
...
void Symbols::parseKernelSymbols(CodeCache* cc) {
    ...
    if (fd == -1) {
        WARNING("open(\"/proc/kallsyms\"): %s", strerror(errno));
        return;
    }

    FILE* f = fdopen(fd, "r");
    if (f == NULL) {
        WARNING("fdopen(): %s", strerror(errno));
        close(fd);
        return;
    }
...
void Symbols::parseLibraries(bool kernel_symbols) {
    MutexLocker ml(_parse_lock);

    CodeCacheArray* array = &_native_libs;
    ...
    if (array->count() >= MAX_NATIVE_LIBS && !_libs_limit_reported) {
        WARNING("Number of parsed libraries reached the limit of %d", MAX_NATIVE_LIBS);
