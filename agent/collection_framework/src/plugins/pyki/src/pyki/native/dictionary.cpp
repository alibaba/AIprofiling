...
Dictionary::Dictionary() {
    ...
    _base_index = _table->base_index = INITIAL_BASE_INDEX;
}
...
unsigned int Dictionary::hash(const char* key, size_t length, u8 kind) {
...
unsigned int Dictionary::lookup(const char* key, size_t length, u8 kind) {
    DictTable* table = _table;
    unsigned int h = hash(key, length, kind);
    ...
    while (true) {
        ...
        for (int c = 0; c < CELLS; c++) {
            if (row->keys[c] == NULL) {
                ...
                if (__sync_bool_compare_and_swap(&row->keys[c], NULL,
                                                 new_key)) {
                    row->lengths[c] = length;
                    row->kinds[c] = kind;
...
void Dictionary::collect(std::map<u32, StringWithLength>& map) {
    collect(map, _table);
}
...
void Dictionary::collect(std::map<u32, StringWithLength>& map,
    ...
    for (int i = 0; i < ROWS; i++) {
        ...
        for (int j = 0; j < CELLS; j++) {
            if (row->keys[j] != NULL) {
                map[table->index(i, j)] = {row->keys[j], row->lengths[j],
                                           row->kinds[j]};
            }
        }
        if (row->next != NULL) {
