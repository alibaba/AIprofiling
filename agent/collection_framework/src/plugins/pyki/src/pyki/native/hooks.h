...
#    define PYKI_PLATFORM_HOOKED_FUNCTIONS
...
#define PYKI_HOOKED_FUNCTIONS                                                                         \
    ...
    PYKI_PLATFORM_HOOKED_FUNCTIONS
    ...
    void ensureValidOriginalSymbol()
    {
        ...
        if (symbol_addr != nullptr) {
            if (symbol_addr != d_original) {
                WARNING("Correcting symbol for %s from %p to %p",
                    d_symbol, reinterpret_cast<void*>(d_original), reinterpret_cast<void*>(symbol_addr));
            }
...
#define PYKI_ORIG_concat_helper(x, y) x##y
#define PYKI_ORIG_NO_NS(f) PYKI_ORIG_concat_helper(pyki_, f)
#define PYKI_ORIG(f) pyki::hooks::PYKI_ORIG_NO_NS(f)

#define FOR_EACH_HOOKED_FUNCTION(f) extern SymbolHook<decltype(&::f)> PYKI_ORIG_NO_NS(f);
