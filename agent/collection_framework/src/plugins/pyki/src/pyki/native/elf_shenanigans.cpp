        ...
        bool restore_original)
{
    ...
    if (unprotect_page(addr) < 0) {
        WARNING("Could not prepare the memory page for symbol %s for patching", symname);
        return;
    }
...
#define FOR_EACH_HOOKED_FUNCTION(hookname)                                                              \
    if (strcmp(PYKI_ORIG(hookname).d_symbol, symname) == 0) {                                         \
        patch_symbol(                                                                                   \
                PYKI_ORIG(hookname),                                                                  \
                &intercept::hookname,                                                                   \
                symname,                                                                                \
...
phdrs_callback(dl_phdr_info* info, [[maybe_unused]] size_t size, void* data) noexcept
{
    ...
    INFO("Patching symbols for %s", info->dlpi_name);
