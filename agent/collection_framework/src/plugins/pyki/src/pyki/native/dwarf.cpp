...
void DwarfParser::parse(const char* eh_frame_hdr) {
    ...
    if (version != 1 || (eh_frame_ptr_enc & 0x7) != 0x3 || (fde_count_enc & 0x7) != 0x3 || (table_enc & 0xf7) != 0x33) {
        WARNING("Unsupported .eh_frame_hdr [%02x%02x%02x%02x] in %s", version,
            ...
            case 0:
                switch (op) {
                    ...
                    case DW_CFA_GNU_args_size:
                        ...
                        break;
                    default:
                        WARNING("Unknown DWARF instruction 0x%x in %s", op, _name);
                        return;
                }
                break;
    ...
    while (_ptr < end) {
        ...
        switch (op) {
            ...
            case DW_OP_plus:
                ...
                break;
            default:
                WARNING("Unknown DWARF opcode 0x%x in %s", op, _name);
                _ptr = end;
                return 0;
        }
    }
