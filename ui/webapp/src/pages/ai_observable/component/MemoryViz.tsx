...
const MemoryViz: React.FC<MemoryVizProps> = ({ snapshotUrl }) => {
  ...
  useEffect(() => {
    ...
    // 初始化界面
    initializeUI(container, snapshotUrl);
  }, [snapshotUrl]);
...
// ====== 以下是从 MemoryViz.js 转换的辅助函数 ======

function version_space() {
  const version: Record<number, number> = {};
  return (addr: number, increment: boolean) => {
    if (!(addr in version)) {
      version[addr] = 0;
    }
...
function addStroke(d: any) {
  d.attr('stroke', 'red')
    .attr('stroke-width', '2')
    .attr('vector-effect', 'non-scaling-stroke');
}

function removeStroke(d: any) {
  d.attr('stroke', '');
}

function format_user_metadata(user_metadata: any) {
  if (!user_metadata) {
    return '';
  }
  ...
  if (
    ...
    Object.keys(user_metadata).length === 0
  ) {
    return '';
  }
  const metadata_lines = Object.entries(user_metadata).map(
    ([key, value]) => `  ${key}: ${value}`
  );
  return 'User Metadata:\n' + metadata_lines.join('\n');
}

function elideRepeats(frames: any[]) {
  const result = [];
...
function frameFilter({ name, filename }: any) {
  const omitFunctions = [
    'unwind::unwind',
...
function format_frames(frames: any[]) {
  if (frames.length === 0) {
    return (
...
function eventStack(e: any, allocated?: number, reserved?: number) {
...
function annotate_snapshot(snapshot: any) {
  snapshot.segment_version = version_space();
  snapshot.block_version = version_space();
  snapshot.categories = [];
  const empty_list: any[] = [];
  let next_stream = 1;
  const stream_names: Record<number, number> = { 0: 0 };
  
  function stream_name(s: number) {
...
function StackInfo(outer: any) {
  const stack_trace = outer
    .append('pre')
    .attr('style', 'grid-column: 1 / 3; grid-row: 2; min-height: 0; overflow-y: auto; overflow-x: hidden; color: #000; background: #fff; padding: 10px; margin: 0; white-space: pre-wrap; word-wrap: break-word; border: 1px solid #ccc;');
  let selected = {
    enter: () => {
      stack_trace.text('');
    },
  ...
  return {
    register(dom: any, enter: any, leave = (_e: any) => {}, select = (_e: any) => {}) {
      dom
        .on('mouseover', function(this: any, _e: any) {
          selected.leave();
          stack_trace.text(enter(d3.select(this)));
        })
        .on('mousedown', function(this: any, _e: any) {
        ...
        .on('mouseleave', function(this: any, _e: any) {
          leave(d3.select(this));
          selected.enter();
        });
    },
    highlight(enter: any, leave = () => {}) {
...
function MemoryView(outer: any, stack_info: any, snapshot: any, device: number) {
  const svg = outer
    .append('svg')
  ...
  const seg_zoom = zoom();
  seg_zoom.on('zoom', function(this: any, event: any) {
    g.attr('transform', event.transform);
  });
  svg.call(seg_zoom);

  const sorted_segments: any[] = [];
  const block_map: Record<number, any> = {};
  ...
  function simulate_memory(idx: number) {
    const l_segments = sorted_segments.map((x) => {
      return { ...x };
    });
    const l_block_map = { ...block_map };

    function map_segment(merge: boolean, seg: any) {
    ...
    function unmap_segment(merge: boolean, seg: any) {
      if (!merge) {
        l_segments.splice(
  ...
  return {
    draw(idx: number) {
      ...
      const segments:any = [...segments_unsorted].sort((x: any, y: any) =>
        x.size === y.size ? x.addr - y.addr : x.size - y.size
      );

      const segments_by_addr = [...segments].sort((x: any, y: any) => x.addr - y.addr);
      ...
      const segments_selection = segment_d
        ...
        .enter()
        .append('rect')
        .attr('x', (x: any) => xScale(x.offset))
        .attr('y', (x: any) => yScale(x.row))
        .attr('width', (x: any) => xScale(x.size))
      ...
      function find_segment(addr: number) {
        let left = 0;
      ...
      const block_selection = block_g
        ...
        .enter()
        .append('rect')
        .attr('x', (x: any) => xScale(x.segment.offset + (x.addr - x.segment.addr)))
        .attr('y', (x: any) => yScale(x.segment.row))
        .attr('width', (x: any) => xScale(x.requested_size))
        .attr('height', yScale(4 / 5))
        .attr('fill', (x: any, _i: number) =>
          x.free_requested
            ? 'red'
            : schemeTableau10[
      ...
      const free_selection = block_r
        ...
        .attr('y', (x: any) => yScale(x.segment.row))
        .attr('width', (x: any) => xScale(x.size - x.requested_size))
        .attr('height', yScale(4 / 5))
        .attr('fill', (_x: any, _i: number) => 'red');
      ...
      const reserved = segments.reduce((x: number, y: any) => x + y.size, 0);
      const allocated = blocks.reduce((x: number, y: any) => x + y.requested_size, 0);
      return [reserved, allocated];
    },
  };
}

function EventSelector(outer: any, events: any[], stack_info: any, memory_view: any) {
  ...
  const events_selection = events_div
    ...
    .enter()
    .append('pre')
    .text((e: any) => formatEvent(e))
    .attr('style', '');

  let selected_event_idx: number | null = null;
  ...
  const es = {
    select(idx: number | null) {
      ...
      if (idx !== null) {
        ...
        // 使用标准的 scrollIntoView 替代非标准的 scrollIntoViewIfNeeded
  ...
  d3.select('body').on('keydown', function(this: any, event: any) {
    const key = event.key;
    const actions: Record<string, number> = { ArrowDown: 1, ArrowUp: -1 };
  ...
  stack_info.register(
    events_selection,
    (t: any) => eventStack(t.datum()),
    (_t: any) => {},
    (d: any) => es.select(d.datum().idx)
  );

  return es;
}

function create_segment_view(dst: any, snapshot: any, device: number) {
  const outer = dst
    .append('div')
    .attr(
      'style',
      'display: grid; grid-template-columns: 1fr 2fr; grid-template-rows: 2fr minmax(0, 1fr); width: 100%; height: 100%; gap: 10px; color: #000; box-sizing: border-box;'
    );
  ...
  // don't auto scroll to end
  //window.requestAnimationFrame(function () {
  //  event_selector.select(events.length > 0 ? events.length - 1 : null);
  //});
}

// 简化版本的 create_trace_view - 用于 Active Memory Timeline
function create_trace_view(
  dst: any,
  snapshot: any,
  device: number,
  plot_segments: boolean = false,
  max_entries: number = 15000
) {
  const left_pad = 70;
  ...
  const d = dst.append('div').attr('style', 'color: #000;');
  d.append('input')
    .attr('type', 'range')
    .attr('min', 0)
  ...
  d.append('label')
    .attr('style', 'margin-left: 10px; color: #000;')
    .text(`Detail: ${max_entries} of ${data.elements_length} entries`);

  const grid_container = dst
    .append('div')
    .attr(
      'style',
      'display: grid; grid-template-columns: 1fr; grid-template-rows: 10fr 1fr 8fr; width: 100%; height: 100%; gap: 10px; color: #000; box-sizing: border-box;'
    );
  ...
  const context_div = grid_container
    ...
    .attr(
      'style',
      'grid-column: 1; grid-row: 3; width: 100%; height: 100%; min-height: 0; overflow-y: auto; overflow-x: hidden; color: #000; background: #fff; border: 1px solid #ccc; padding: 10px; box-sizing: border-box;'
    );
  const delegate = ContextViewer(
    context_div.append('pre').attr('style', 'margin: 0; white-space: pre-wrap; word-wrap: break-word;').text('none'),
    data
  );
  plot.set_delegate(delegate);
}

function process_alloc_data(
  snapshot: any,
  device: number,
  ...
  const elements: any[] = [];
  const initially_allocated: number[] = [];
  const actions: number[] = [];
  const addr_to_alloc: Record<number, number> = {};
  ...
  const summarized_elems: Record<number, any> = {};
  ...
  const draw_elem: Record<number, boolean> = {};
  for (const [_s, e] of sizes.slice(0, max_entries)) {
    draw_elem[e as number] = true;
  }

  function add_allocation(elem: number) {
...
function MemoryPlot(
  ...
  function format_points(d: any) {
    const size = d.size;
    const xs = d.timesteps.map((t: number) => xscale(t));
    const bottom = d.offsets.map((t: number) => yscale(t));
    const m = Array.isArray(size)
      ? (t: number, i: number) => yscale(t + size[i])
      : (t: number) => yscale(t + size);
    const top = d.offsets.map(m);
    const p0 = xs.map((x: number, i: number) => `${x},${bottom[i]}`);
    const p1 = xs.map((x: number, i: number) => `${x},${top[i]}`).reverse();
  ...
  const plot = scrub_group
    ...
    .attr('fill', (d: any) => colors[d.color % colors.length]);

  const axis = plot_coordinate_space.append('g').call(yaxis);

  function handleZoom(this: any, event: any) {
  ...
  return {
    select_window: (stepbegin: number, stepend: number, max: number) => {
      const begin = xscale(stepbegin);
      const size = xscale(stepend) - xscale(stepbegin);
      const scale = plot_width / size;
      const translate = -begin;
      const yscale_ratio = max_size / max;
      scrub_group.attr(
        'transform',
        `scale(${scale / yscale_ratio}, 1) translate(${translate}, 0)`
      );
      plot_outer.call(
        thezoom.transform,
        zoomIdentity
          .scale(yscale_ratio)
          .translate(0, -(plot_height - plot_height / yscale_ratio))
      );
    },
    set_delegate: (delegate: any) => {
      plot
        .on('mouseover', function (this: any, _e: any, _d: any) {
          delegate.set_selected(d3.select(this));
        })
        .on('mousedown', function (this: any, _e: any, _d: any) {
          delegate.default_selected = d3.select(this);
        })
        .on('mouseleave', function (this: any, _e: any, _d: any) {
...
function ContextViewer(text: any, data: any) {
  let current_selected: any = null;
...
function MiniMap(
  ...
  brush.on('brush', function (event: any) {
    const [begin, end] = event.selection.map((x: number) => x - left_pad);
...
function Legend(plot_svg: any, categories: string[]) {
  const xstart = 100;
  const ystart = 5;
  plot_svg
    .append('g')
    ...
    .enter()
    .append('rect')
    .attr('x', (_c: string, _i: number) => xstart)
    .attr('y', (_c: string, i: number) => ystart + i * 15)
    .attr('width', 10)
    .attr('height', 10)
    .attr('fill', (_c: string, i: number) => schemeTableau10[i % schemeTableau10.length]);
  plot_svg
    .append('g')
    .selectAll('text')
    .data(categories)
    .enter()
    .append('text')
    .attr('x', (_c: string, _i: number) => xstart + 20)
    .attr('y', (_c: string, i: number) => ystart + i * 15 + 8)
...
// 简化版本的 create_settings_view - 用于 Allocator Settings
function create_settings_view(dst: any, snapshot: any, device: number) {
  dst.selectAll('svg').remove();
  dst.selectAll('div').remove();
  const settings_div = dst.append('div').attr('style', 'padding: 20px; color: #000; height: 100%; overflow: auto; box-sizing: border-box;');
  settings_div.append('h3').text('CUDA Caching Allocator Settings:');

  if ('allocator_settings' in snapshot) {
    settings_div
      .append('pre')
      .attr('style', 'color: #000; background: #f5f5f5; padding: 10px; border-radius: 4px; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word; max-width: 100%;')
      .text(JSON.stringify(snapshot.allocator_settings, null, 2));
  } else {
    settings_div
      .append('p')
      .text('No allocator settings found in this snapshot.');
  }
}

async function decompressGzip(arrayBuffer: ArrayBuffer) {
  ...
  const bytes = new Uint8Array(
    chunks.reduce((acc: number, chunk: Uint8Array) => acc + chunk.length, 0)
  );
  let offset = 0;
...
function unpickle(buffer: ArrayBuffer) {
  try {
...
function unpickleData(buffer: ArrayBuffer): any {
  ...
  function setitems(d: any, mark: number) {
      ...
      case LONG1:
        {
          ...
          if (s <= 8) {
            ...
            // BigInt
            let scratch_bytes_unbounded: number[] = [];
            for (let i = 0; i < s; i++) {
            ...
            if (negative) {
              ...
              for (let i = 0; i < s; i++) {
                const twos_complement =
                  (0xff ^ scratch_bytes_unbounded[i]) + carry;
                carry = twos_complement > 0xff ? 1 : 0;
                scratch_bytes_unbounded[i] = 0xff & twos_complement;
              }
            }

            const hex_str = Array.from(
              scratch_bytes_unbounded.reverse(),
              (byte) => {
                return byte.toString(16).padStart(2, '0');
              }
            ).join('');

            const big_int = negative
              ? -BigInt(`0x${hex_str}`)
              : BigInt(`0x${hex_str}`);
            stack.push(big_int);
          }
...
function initializeUI(container: any, snapshotUrl: string) {
  const style = `
    pre {
      margin: 0px;
      color: #000;
  ...
  const controls = container
    .append('div')
    .attr('style', 'display: flex; gap: 10px; padding: 5px; flex-shrink: 0; align-items: center;');

  const kinds: Record<string, Function> = {
    'Active Memory Timeline': create_trace_view,
    'Allocator State History': create_segment_view,
    'Active Cached Segment Timeline': (dst: any, snapshot: any, device: number) =>
      create_trace_view(dst, snapshot, device, true),
    'Allocator Settings': create_settings_view,
  };

  const view = controls.append('select').style('color', '#000').style('background', '#fff').style('flex', '0 0 auto');
  for (const x in kinds) {
    view.append('option').text(x);
  }
  
  const gpu = controls.append('select').style('color', '#000').style('background', '#fff').style('flex', '0 0 auto');
  gpu.style('visibility', 'hidden');
  
  const snapshot_select = controls.append('select').style('color', '#000').style('background', '#fff').style('flex', '0 0 auto');
  snapshot_select.style('visibility', 'hidden');

  // 创建内容区域
  const content_area = container
    .append('div')
    .attr('style', 'flex: 1; overflow: hidden; position: relative;');

  const snapshot_cache: Record<string, any> = {};
  const snapshot_to_loader: Record<string, Function> = {};
  const selection_to_div: Record<string, any> = {};

  function snapshot_change(f: string) {
    ...
    const has_segments: Record<number, boolean> = {};
    ...
    const key = [f, view_value, device].join('_');
    if (!(key in selection_to_div)) {
      selection_to_div[key] = content_area.append('div').attr('style', 'width: 100%; height: 100%; position: absolute; top: 0; left: 0;');
      kinds[view_value](selection_to_div[key], snapshot, device);
    }
    const selected_div = selection_to_div[key];

    selected_div.attr('style', 'display: block; width: 100%; height: 100%; position: absolute; top: 0; left: 0;');
  }

  function selected_change() {
    for (const d of Object.values(selection_to_div)) {
      d.attr('style', 'display: none; width: 100%; height: 100%; position: absolute; top: 0; left: 0;');
    }
  ...
  function add_snapshot(name: string, loader: Function) {
    snapshot_select.append('option').text(name);
    snapshot_to_loader[name] = loader;
  }

  function finished_loading(name: string, data: ArrayBuffer) {
    const unpickled = unpickle(data);
    annotate_snapshot(unpickled);
    snapshot_cache[name] = unpickled;
    snapshot_change(name);
  }

  // 加载远程文件
  add_snapshot('oom_snapshot', (unique_name: string) => {
